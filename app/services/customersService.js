...
app.service('customersService', function () {
    ...
    // send data chat
    this.sendChat = function(data){
        console.log('send data to server')
    }

    // send data search
    this.searchBox = function(keywork){
        console.log(keywork)
        var arr = [];
        if(keywork != ''){
            for (var i = 0; i < customers.length; i++) {
                if((customers[i].firstName.toLowerCase()).indexOf(keywork.toLowerCase()) > 0 || (customers[i].lastName.toLowerCase()).indexOf(keywork.toLowerCase()) > 0 || (customers[i].city.toLowerCase()).indexOf(keywork.toLowerCase()) > 0 ){
